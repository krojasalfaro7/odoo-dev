...
#Métodos necesarios extraidos de web/controllers/main.py
...
"""
	#Extendiendo el método web para redireccionar la base de datos
	@http.route('/web', type='http', auth="none")
    def web_client(self, s_action=None, **kw):
        #Verificando que la base de datos sea correcta
        ensure_db()
        #Se desea que se redireccione a la página principal y no a la de login
        if not request.session.uid:
            return werkzeug.utils.redirect('/', 303)
        if kw.get('redirect'):
            return werkzeug.utils.redirect(kw.get('redirect'), 303)
